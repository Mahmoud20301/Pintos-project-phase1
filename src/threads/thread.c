...
void thread_tick(void)
{
  ...
  if (++thread_ticks >= TIME_SLICE)
    intr_yield_on_return();   //This yields the CPU after the interrupt completes, allowing the scheduler to pick a more appropriate thread.
...
void set_sleeping_threads(int64_t ticks)
{
  ...
  list_insert_ordered(&sleep_list, &cur->elemSleep, compare_priority_sleep, NULL);
...
{
  enum intr_level old_level = intr_disable();
  bool result = !list_empty(&ready_list) && (list_entry(list_min(&ready_list, thread_priority_more, NULL), struct thread, elem)->priority > thread_get_priority());
  intr_set_level(old_level);
  if (result)
  {
    thread_yield();
...
void thread_unblock(struct thread *t)
{
  ...
  ////////////////////////// replaced list_push_back(&ready_list, &t->elem) to use priority ordered ready list//////////////////
...
init_thread(struct thread *t, const char *name, int priority)
{
  ...
  list_init(&t->locks_held);           /*Initializes the held locks list*/
  t->waiting_on_lock = NULL;           /*Not waiting on any lock initially*/ 
  //////////////////////////////////////////////////////////////////////////
...
next_thread_to_run(void)
{
  ...
  {
    struct thread *max_thread = list_entry(list_min(&ready_list, thread_priority_more, NULL), struct thread, elem);
...
void calculate_load_avg()
{
    ...
    Fixed_P term1 = fixed_mul(fixed_div_int(fixed_from_int(59), 60), load_avg);
...
void calculate_recent_cpu(struct thread *t, void *aux UNUSED)
{
  ...
  calculate_dynamic_priority(t, NULL);
}

// increment recent_cpu for current thread by 1
...
//////////////////////////
void calculate_dynamic_priority(struct thread *t, void *aux UNUSED)
{
  ASSERT(is_thread(t));
  if (t == idle_thread)
    return;

  Fixed_P recent_cpu_component = fixed_div_int(t->recent_cpu, 4);
  Fixed_P nice_component = fixed_from_int(2 * t->nice);
  Fixed_P raw_priority = fixed_sub(fixed_from_int(PRI_MAX), fixed_add(recent_cpu_component, nice_component));

  t->priority = fixed_to_int_floor(raw_priority);
...
bool compare_priority_sleep(const struct list_elem *a,
