...
thread_tick (void) 
{
  ...
  if (t == idle_thread)
    idle_ticks++;
  else if (t != idle_thread)                         //increment recent_cpu of running threads each 1 tick
  {                                
...
thread_set_nice (int nice UNUSED) 
{
  ...
  calculate_dynamic_priority(thread_current(), NULL);           //completed
  thread_yield();
}
...
{
  return thread_current()->nice;                                //completed
}
...
next_thread_to_run(void) {
  if (list_empty(&ready_list))
    return idle_thread;
  if (thread_mlfqs) {
    // MLFQS mode: select the thread with the highest dynamic priority
    struct thread *max_thread = list_entry(
    list_min(&ready_list, compare_dynamic_priority, NULL),
      struct thread, elem);
    list_remove(&max_thread->elem);
    return max_thread;
  } 
  else {
    // select the next thread in FIFO order
    return list_entry(list_pop_front(&ready_list), struct thread, elem);
  }
}
bool compare_dynamic_priority(const struct list_elem *a, const struct list_elem *b, void *aux UNUSED) {
  const struct thread *thread_a = list_entry(a, struct thread, elem);
  const struct thread *thread_b = list_entry(b, struct thread, elem);
  return thread_a-> dynamic_priority > thread_b-> dynamic_priority;
}
...
void calculate_dynamic_priority(struct thread *t, void *aux UNUSED)
{
  ...
  Fixed_P nice_component = fixed_from_int(t->nice * 2);
  ...
  if (t->dynamic_priority > PRI_MAX)
    t->dynamic_priority = PRI_MAX; 
  else if (t->dynamic_priority < PRI_MIN)
    t->dynamic_priority = PRI_MIN; 
}
